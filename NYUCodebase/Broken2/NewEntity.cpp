...
void Entity::FixedUpdateX() {
    velocity_x = lerp(velocity_x, 0.0f, 0.0166666f * friction_x);
    velocity_x += 0.0166666f * acceleration_x;
    x += 0.0166666f * velocity_x;
}

void Entity::FixedUpdateY() {
    if (!collidedBottom) {acceleration_y = -4.0f;}
    velocity_y = lerp(velocity_y, 0.0f, 0.0166666f * friction_y);
    velocity_y += 0.0166666f * acceleration_y;
    if (velocity_y < -2.0) {velocity_y = -2.0;}
    y += 0.0166666f * velocity_y;
}
void Entity::Render(float elapsed) {
    ...
    if (type == "Button"&& x != 0 && y != 0) {
        if (collidedTop) {
            DrawSprite(LoadTexture("button.png"), 1, 2.0, 1.0);
        }
        DrawSprite(LoadTexture("button.png"), 0, 1.0, 1.0);
    }
